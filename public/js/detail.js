(() => {
    ...
    async function mount() {
        try {
            const App = await loadModule('/components/EntityDashboard.vue', options);
            window.Vue.createApp(App).mount('#app');
        }
        catch (err) {
            console.error('Failed to mount detail app:', err);
            const el = document.getElementById('app');
            if (el) {
                el.innerHTML = '<p style="padding:1rem">Failed to load details.</p>';
            }
        }
    }
